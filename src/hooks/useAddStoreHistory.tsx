...
export async function addStoreHistory(data: any, narxi = '', location: any) {
  ...
  try {
    location === '/mahsulot-sotib-olish' || location === '/mahsulot-sotish'
      ? await setDoc(doc(db, 'storage2', 'zWcuaTnZqglflTVQHkAE'), {
          storeHistory: [...JSON?.parse(localStorage?.getItem('storeHistory') as any), historyData],
        })
      : location === '/oziq-ovqat-uchun-chiqim'
      ? await setDoc(doc(db, 'storage2', 'YVkxqmCuab7CedFFrMIU'), {
          oziqOvqatChiqim: [...JSON?.parse(localStorage?.getItem('oziqOvqatChiqim') as any), historyData],
        })
      : await setDoc(doc(db, 'storage2', 'MCK41JPlggbyt5BSkqCx'), {
          korxonaUchunChiqim: [...JSON?.parse(localStorage?.getItem('korxonaUchunChiqim') as any), historyData],
        })
  } catch (err) {
    console.log(err)
  }
