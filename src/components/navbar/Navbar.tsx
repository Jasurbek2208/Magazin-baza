...
// Components
import { Loading, Button, Input, Select } from 'components'

const Navbar = memo(({ currentuser, admins, isProfileOpen, setIsProfileOpen }: any) => {
  ...
  async function saveEditingProfile(data) {
    ...
    try {
      ...
      admins.forEach((i) => (i.id !== currentuser.id ? oldDatas.push(i) : oldDatas.push(data)))
      ...
      toast.success("Profil o'zgartirishda xatolik yuz berdi. Iltimos, qaytadan urinib ko'ring!")
    } finally {
      setDisbl(false)
      setIsLoading(false)
    }
  ...
  function uploadImage(e) {
    ...
    const urlName = 'adminsAvatar/' + currentuser?.firstName + '_' + currentuser?.lastName + '_avatar-' + v4()
  ...
  async function editAccount(currentAdmin, currEdited) {
    ...
    await signInWithEmailAndPassword(auth, currentAdmin.email, currentAdmin.password).then((userCredential) => {
      userCredential.user.delete()
    })

    // Register new account
    await createUserWithEmailAndPassword(auth, currEdited.email, currEdited.password).then((userCredential) => {
  ...
  return (
    <StyledNavbar image={image} avatar={currentuser?.avatar}>
      <div className='container'>
        <div className='user_profile' onClick={() => setIsProfileOpen((p) => !p)}>
      ...
      {isProfileOpen && (
        <div className={'user-profile-modal' + (isClosingTime ? ' closing' : '')}>
          <div className={'modal-close-btn' + (editProfile ? ' off' : '')}>
            <i className='icon fa-solid fa-close' onClick={() => (avatarView ? setAvatarView(false) : setIsClosingTime(true))}></i>
          </div>
          {!editProfile ? (
            <div className={'user-info__wrapper' + (avatarView ? ' On' : '')}>
              ...
              <h2 className='user-fullname'>{currentuser?.firstName + ' ' + currentuser?.lastName}</h2>
              ...
              <div className='button__wrapper'>
                <Button content='Edit profile' width='100%' onClick={currentValidation} />
              </div>
            </div>
          ) : (
            <div className='info-edit user-info__wrapper'>
              <div className='user_profile'>
                <input title='Rasm yuklash' type='file' name='admin-avatar' id='admin-avatar' onChange={uploadImage} />
                <i className='icon fa-solid fa-user'></i>
              </div>
              <form onSubmit={handleSubmit(saveEditingProfile)} className='user-edit-form'>
                ...
                <div className='input__wrapper'>
                  <Select label='Jinsi:' list={['erkak', 'ayol']} sortData={setGenre} selected={genre} isFormSelect />
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (!e.target.validity.valid) {
                        e.target.setCustomValidity("Raqam noto'g'ri kiritilgan ! Misol: +998-97-105-05-05")
                      }
                    }}
                  />
                </div>
                ...
                <div className='button__wrapper'>
                  <Button type='submit' content='Save' width='100%' disbl={disbl} />
                </div>
                <div className='button__wrapper'>
                  <Button type='button' content='Cancel' width='100%' disbl={disbl} onClick={() => resetForm(true)} />
                </div>
              </form>
            </div>
          )}
