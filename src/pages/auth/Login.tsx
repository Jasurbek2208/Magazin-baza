    ...
    <StyledLogin>
      <main className='login__wrapper'>
        ...
        <form onSubmit={handleSubmit(userLogin)} className='form__wrapper'>
          ...
          />
          {errors?.email && <span className='error'>{(errors?.email as any)?.message || ''}</span>}
          <Input
            onChange={() => setError(false)}
          ...
          {error && (
            <span className='error' style={{ textAlign: 'center', fontWeight: 600 }}>
