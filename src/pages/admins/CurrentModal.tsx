...
const CurrentModal = memo(({ admins, currentAdmin, isClose }: any) => {
  ...
  async function deleteCurrPartner() {
    setDisbl(true)
    const oldDatas = admins?.filter((i: any) => i.id !== currentAdmin?.id)
    ...
    try {
      ...
      signInWithEmailAndPassword(auth, currentAdmin?.email, currentAdmin?.password)
        .then((userCredential) => {
  ...
  async function editAccount() {
    ...
    await createUserWithEmailAndPassword(auth, currEditedPartner.email, currEditedPartner.password)
      .then((userCredential) => {
  ...
  async function saveEditedPartner() {
    ...
    if (currentAdmin?.email !== currEditedPartner.email || currentAdmin?.password !== currEditedPartner.password) await editAccount()

    admins?.forEach((i: any) => (i.id !== currentAdmin?.id ? oldDatas.push(i) : oldDatas.push(currEditedPartner)))

    try {
      await setDoc(doc(db, 'users', 'hjJzOpbuR3XqjX817DGvJMG3Xr82'), { admins: oldDatas })
  ...
  function valueOnChanged(name: string, value: string) {
    setCurrEditedPartner((p: any) => ({ ...p, [name]: value }))
  }
        ...
        <form onSubmit={handleSubmit(saveEditedPartner)}>
          <div className='info__wrapper'>
            <div className={'info' + (isEdited ? ' edited' : '')}>
              <h5>Ism:</h5>
              {!isEdited ? (
                <p className={'information' + (isEdited ? ' edited' : '')}>{currentAdmin?.firstName}</p>
              ) : (
            ...
            <div className={'info' + (isEdited ? ' edited' : '')}>
              <h5>Familiya:</h5>
              {!isEdited ? (
                <p className={'information' + (isEdited ? ' edited' : '')}>{currentAdmin?.lastName}</p>
              ) : (
                <div className='input__wrapper'>
                  <Input
            ...
            <div className={'info' + (isEdited ? ' edited' : '')}>
              <h5>Jinsi:</h5>
              {!isEdited ? (
                <p className={'information' + (isEdited ? ' edited' : '')}>{currentAdmin?.genre}</p>
              ) : (
                <div className='input__wrapper'>
                  <Select list={['erkak', 'ayol']} sortData={setGenre} selected={genre} isFormSelect />
                </div>
              )}
            </div>
            <div className={'info' + (isEdited ? ' edited' : '')}>
              <h5>Email:</h5>
              {!isEdited ? (
                <p className={'information' + (isEdited ? ' edited' : '')}>{currentAdmin?.email}</p>
              ) : (
                <div className='input__wrapper'>
                  <Input
            ...
            <div className={'info' + (isEdited ? ' edited' : '')}>
              <h5>Parol:</h5>
              {!isEdited ? (
                <p className={'information' + (isEdited ? ' edited' : '')}>{currentAdmin?.password}</p>
              ) : (
                <div className='input__wrapper'>
                  <Input
            ...
            <div className={'info' + (isEdited ? ' edited' : '')}>
              <h5>Telefon raqam:</h5>
              {!isEdited ? (
                <p className={'information' + (isEdited ? ' edited' : '')}>{currentAdmin?.phoneNumber}</p>
              ) : (
                <div className='input__wrapper'>
                  <Input
                    require
            ...
            <div className={'info' + (isEdited ? ' edited' : '')}>
              <h5>Lavozimi:</h5>
              <p title={isEdited ? "Lavozimni o'zgartirish mumkin emas!" : ''} className='information'>
                {currentAdmin?.rol?.map((rol: string, idx: number) => (
          ...
          <div className='actions__wrapper'>
            <div className='button__wrapper'>
              <Button type={isEdited ? 'submit' : 'button'} content={isEdited ? 'Save' : 'Close'} width='100%' disbl={disbl} onClick={() => (isEdited ? null : isClose(false))} />
            </div>
            ...
            <div className='button__wrapper'>
              <Button content='Delete' width='100%' disbl={disbl || isEdited} onClick={deleteCurrPartner} />
            </div>
          </div>
        </form>
      </div>
